...
def _get_in_group(context, group):
    if context.__class__ == Profile:
        ...
        if 'group.KarlStaff' in user['groups']:
            return True
    return False
def _get_is_karlstaff(context):
    return _get_in_group(context, 'group.KarlStaff')
def _get_is_karlformerstaff(context):
    return _get_in_group(context, 'group.KarlFormerStaff')
...
metadata_fields = [
    ...
    ('is_karlstaff', _get_is_karlstaff),
    ('is_karlformerstaff', _get_is_karlformerstaff),
    #('tags', get_creator),
]

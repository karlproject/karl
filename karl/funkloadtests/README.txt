...
Installation
=======================
Read INSTALL.txt in this directory for install and fix of cookie glitch.

These next instructions are adapted from the "simple FunkLoadTestCase demonstration" that came with Funkload.
...
1/ Modify the community.conf file
...
2/ Test it
   ...
   verbose mode::
     ...
     fl-run-test -v community.py

   debug mode::

     fl-run-test -d community.py
   ...
   check performance of a single page::
     ...
     fl-run-test -l 4 -n 100 community.py

   see what tests are in a file::
    fl-run-test community.py --list
...
3/ Bench it
   ...
   Bench it with few cycles::
     ...
     fl-run-bench -c 1:25:50:75 community.py Community.test_10_create_comm

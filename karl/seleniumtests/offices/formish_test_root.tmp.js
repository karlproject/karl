module.exports = function testFormishTestRoot (browser, lbParam, verificationErrors)  {

    if (!lbParam) lbParam = {vuSn: 1};
    var assert = require('assert');
    var baseUrl = "http://change-this-to-the-site-you-are-testing/";
    var acceptNextAlert = true;
    browser.get(addUrl(baseUrl, "/offices/edit.html"));
    /* ERROR: Caught exception [Error: locator strategy either id or name must be specified explicitly.] */
    /* ERROR: Caught exception [Error: locator strategy either id or name must be specified explicitly.] */
    /* Warning: assertTextPresent may require manual changes */
    assert.strictEqual(browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Title \n is required[\\s\\S]*$"), true, 'Assertion error: Expected: true, Actual:' browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Title \n is required[\\s\\S]*$"));
    /* ERROR: Caught exception [Error: locator strategy either id or name must be specified explicitly.] */
    /* ERROR: Caught exception [Error: locator strategy either id or name must be specified explicitly.] */
    /* ERROR: Caught exception [Error: locator strategy either id or name must be specified explicitly.] */
    assert.equal("is required", browser.elementByXPath("//div[@id='save-description--field']/span").text(), 'Assertion error: Expected: "is required", Actual: ' + browser.elementByXPath("//div[@id='save-description--field']/span").text());

module.exports = function testOfficesSearch (browser, lbParam, verificationErrors)  {
    ...
    browser.elementByXPath("//form[@id='kcommunitysearch']/fieldset/input[1]").clear();
    browser.elementByXPath("//form[@id='kcommunitysearch']/fieldset/input[1]").sendKeys("selenium_test");
    browser.elementByXPath("//button[@class='search-community-button']").click();
    /* Warning: assertTextPresent may require manual changes */
    assert.strictEqual(browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Search Results[\\s\\S]*$"), true, 'Assertion error: Expected: true, Actual:' browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Search Results[\\s\\S]*$"));

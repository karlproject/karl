module.exports = function testFilesDeleteFolder (browser, lbParam, verificationErrors)  {
    ...
    browser.elementByLinkText("test folder").click();
    browser.elementByLinkText("Delete").click();
    browser.elementByLinkText("ok").click();
    /* Warning: assertTextPresent may require manual changes */
    assert.strictEqual(browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Deleted test folder[\\s\\S]*$"), true, 'Assertion error: Expected: true, Actual:' browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Deleted test folder[\\s\\S]*$"));

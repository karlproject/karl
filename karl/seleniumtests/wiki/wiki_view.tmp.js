module.exports = function testWikiView (browser, lbParam, verificationErrors)  {
    ...
    assert.strictEqual(browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*selenium_test Community Wiki Page[\\s\\S]*$"), true, 'Assertion error: Expected: true, Actual:' browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*selenium_test Community Wiki Page[\\s\\S]*$"));

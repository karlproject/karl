module.exports = function testWiki (browser, lbParam, verificationErrors)  {

    if (!lbParam) lbParam = {vuSn: 1};
    var assert = require('assert');
    var baseUrl = "http://change-this-to-the-site-you-are-testing/";
    var acceptNextAlert = true;
    browser.get(addUrl(baseUrl, "/communities/selenium_test/members/add_existing.html?status_message=Community+added"));
    browser.elementByLinkText("OVERVIEW").click();
    browser.elementByLinkText("Front Page").click();
    ...
    browser.elementByLinkText("+").click();
    /* ERROR: Caught exception [Error: locator strategy either id or name must be specified explicitly.] */
    /* ERROR: Caught exception [Error: locator strategy either id or name must be specified explicitly.] */
    /* Warning: assertTextPresent may require manual changes */
    assert.strictEqual(browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Wiki Page created[\\s\\S]*$"), true, 'Assertion error: Expected: true, Actual:' browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Wiki Page created[\\s\\S]*$"));

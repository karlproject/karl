module.exports = function testMakeCommunity (browser, lbParam, verificationErrors)  {

    if (!lbParam) lbParam = {vuSn: 1};
    var assert = require('assert');
    var baseUrl = "http://change-this-to-the-site-you-are-testing/";
    var acceptNextAlert = true;
    browser.get(addUrl(baseUrl, "/communities"));
    browser.elementByLinkText("Add Community").click();
    browser.elementByName("title").clear();
    browser.elementByName("title").sendKeys("selenium_test");
    browser.elementByName("description").clear();
    browser.elementByName("description").sendKeys("This is a test of Karl Under Selenium.");
    browser.elementByXPath("//input[@name='security_state' and @value='private']").click();
    browser.elementByName("submit").click();
    /* Warning: assertTextPresent may require manual changes */
    assert.strictEqual(browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Community added[\\s\\S]*$"), true, 'Assertion error: Expected: true, Actual:' browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Community added[\\s\\S]*$"));

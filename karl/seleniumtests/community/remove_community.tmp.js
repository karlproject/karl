module.exports = function testRemoveCommunity (browser, lbParam, verificationErrors)  {
    ...
    try {
        assert.strictEqual(browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Do you really want to delete selenium_test [\\s\\S][\\s\\S]*$"), true, 'Assertion error: Expected: true, Actual:' browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Do you really want to delete selenium_test [\\s\\S][\\s\\S]*$"));
    } catch (e) {

module.exports = function testForumsDelete (browser, lbParam, verificationErrors)  {

    if (!lbParam) lbParam = {vuSn: 1};
    var assert = require('assert');
    var baseUrl = "http://change-this-to-the-site-you-are-testing/";
    var acceptNextAlert = true;
    browser.get(addUrl(baseUrl, "/offices/forums"));
    browser.elementByLinkText("selenium_test1").click();
    browser.elementByLinkText("Delete").click();
    browser.elementByLinkText("ok").click();
    /* Warning: assertTextPresent may require manual changes */
    assert.strictEqual(browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Deleted selenium_test1[\\s\\S]*$"), true, 'Assertion error: Expected: true, Actual:' browser.elementByCssSelector("BODY").text().match("^[\\s\\S]*Deleted selenium_test1[\\s\\S]*$"));

...
/*globals jQuery Mustache Image plupload */
...
(function ($) {
    ...
    $.widget('karl.karlmultifileupload', {
        ...
        _create: function () {
            ...
            this.dialogSnippet = $(
                '<div class="karl-multifileupload-dialog-content">' +
                    '<div class="uploader">' +
                        '<p>You browser doesn\'t have Flash, Silverlight, ' +
                        'Gears, BrowserPlus or HTML5 support.</p>' + 
                    '</div>' +
                '</div>'
            );
            this.dialogSnippet
                    ...
                    open: function () {
                        ...
                        if (self.uploader) {
                            ...
                            if (! header_content.data('plupload_mogrified')) {
                                var actualimage_src = header_content.css('background-image')
                                    .replace(/"/g, "").replace(/url\(|\)$/ig, "");
                ...
                if (self.uploader.runtime == 'flash') {
                    $.each(files, function (index) {
                        if (this.size === 0) {
                            ...
                            var error_html = "<strong>Cannot upload a zero-sized file: " +
                                    this.name + "</strong><br>";
        ...
        destroy: function () {
            ...
            this.dialogSnippet.dialog('destroy');
            this.uploader.stop();
        ...
        checkResponse: function (up, file, response) {
            ...
            if (data.error) {
                ...
                var error_html = "<strong>Error during upload of " +
                        err_file.name + ":</strong><br>" + data.error;

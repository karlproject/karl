...
def populate(root, data, do_transaction_begin=True):
    ...
    request.environ['repoze.who.identity'] = {
            'repoze.who.userid': data.who_admin,
    ...
    offices.title = data.offices_title

    offices.__acl__ = data.offices_acl
    offices.feature = data.office_data.sample_feature
    alsoProvides(offices, IIntranets)
    #_reindex(offices)

    from karl.content.views.forum import add_forum_view
    for office in data.office_data.offices:
        request.POST.clear()
        request.POST['form.submitted'] = True
